...
- 寄付金画面では、最終読込した Excel のワークブック名・シート名・読込日時を `DonationMoneyState` で保持し、ページタイトル右側に1行で表示する
- 寄付品画面でも、最終読込した Excel のワークブック名・シート名・読込日時を `DonationGoodsState` で保持し、ページタイトル右側に1行で表示する
- 印刷画面の操作パネルでは見出し説明文を削除し、操作ボタン中心のコンパクト表示にしている
- Excel 取り込みパネルは最大幅を制限し、横方向に間延びしないようにしている
- Excel 取り込みパネルと詳細設定パネルは排他的に開くようにする
- トグルで表示するパネル内には閉じるボタンを置き、パネル内部から閉じられるようにしている
- 寄付金の並べ替え設定パネルは、Excel 取り込みパネルと同じ最大幅で中央表示している
- 設定エクスポート / 設定インポート / 設定初期化の操作は、上部バーではなく詳細設定パネル内に配置している
- ページ再計算の操作ラベルは「ページ割り復元」を使用している
- 寄付金の印刷画面は、データ未読込・0件時に警告メッセージを表示する
- 寄付金のシート読み込み失敗時は、A3 以降のデータ有無、ヘッダー不足、スキップ件数など原因を示す詳細メッセージを返す
